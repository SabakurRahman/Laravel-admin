    ...
    <div class="col-md-4 mb-3">
        ...
        <select name="category_id" class="form-select">
            ...
            @foreach($categories as $category)
                <option
                    {{ isset($filter['category_id']) && $filter['category_id'] == $category->id ? 'selected': null}} value="{{$category->id}}">{{$category->getFullHierarchyAttribute()}}</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-4 mb-3">
...
<div class="row justify-content-center mb-5">
    <div class="col-md-3">
        <div class="d-grid">
            <button id="reset_search_form" type="reset" class="btn btn-outline-warning"><i class="ri-refresh-line"></i>
                Reset Filters
            </button>
        </div>
    </div>
